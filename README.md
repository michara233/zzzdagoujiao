...
一个基于 Python + PyQt5 的 Windows 时间管理程序。它在后台监测前台窗口（或运行进程），当目标窗口/进程连续存在达到设定时长后，在屏幕指定位置弹出无边框、置顶、透明背景的提醒图片，并播放一次音效。
...
- 两种检测模式：
  - 仅检测前台窗口（默认）
  - 检测所有运行进程（仅进程名，不检测标题）
- 达到时长阈值后显示提醒并播放音效（单次、不循环）
- 目标消失后立即隐藏提醒、清零计时、停止音效
- 图形化设置界面，保存后立即生效，配置持久化到 `config.json`
- 系统托盘图标：显示/隐藏提醒、打开设置、退出程序
- 提醒窗口：无边框、置顶、透明背景、可拖动、支持鼠标穿透
- 屏幕变暗：提醒弹出时全屏覆盖半透明遮罩，不透明度可调（0~100%）
...
1. 获取程序：
   - **安装版**：双击 `default.exe`安装，完成后从桌面/开始菜单启动。
2. 启动后，程序会弹出设置界面，系统托盘出现图标（程序常驻后台）。
3. 在设置界面配置你想要的目标（进程名或窗口标题关键词）、触发时长等，点「保存」即可立即生效。
4. 关闭设置窗口后程序继续在后台监测；可通过托盘图标随时再打开设置、显示/隐藏提醒或退出。
...
然后执行以下命令打包（单文件、无控制台窗口、命名提醒）：

```bash
pyinstaller --noconfirm --clean --onefile --windowed --name 提醒 main.py
```
...
- 注意：程序默认使用 `dagou.png` 作为提醒图片、`dagoujiao.MP3` 作为默认音效（相对路径会解析为 exe 同目录），默认位置为屏幕居中。若指定了其它图片或音效文件，请将它们与 exe 放同一目录，或在设置界面用“浏览”选择绝对路径。
...
2. 打开记事本并保持前台，10 秒后提醒弹出并播放音效。
3. 切换到其它窗口，提醒立即隐藏；切回记事本重新计时。
4. 将阈值改为 5 秒并保存，切回记事本后 5 秒弹出。
5. 拖动提醒到其它位置，关闭并重开程序，位置保持（保存为自定义坐标）。
